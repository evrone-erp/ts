...
- По аналогии для работы с jira нужно создать приложение по ссылке https://developer.atlassian.com/console/myapps/. В Permissions должны
  быть выданы все разрешения, перечисленные в `config.json` в `jiraAuth.scope`, за исключением `offline_access`.
